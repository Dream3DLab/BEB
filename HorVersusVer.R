...
working_directory <- "/Users/majbrittbuchholz/Desktop/Stats_horvsver"
...
#60 needs to be changed, it is not horizontal; find right file!
Axis_Lengths$Orientation <- ifelse(grepl("Exp060", Axis_Lengths$Orientation), "Horizontal", Axis_Lengths$Orientation)
...
p <- p +
  annotate("text", x = c(1.5, 3.5), 
           y = 107, 
           label = c(ifelse(p_value_HUVEC < 0.05, "*", "NS"), ifelse(p_value_MCF10A < 0.05, "*", "NS")), 
           color = c(ifelse(p_value_HUVEC < 0.05, "red", "black"), ifelse(p_value_MCF10A < 0.05, "red", "black")), 
           size = 6)
